...
--   OR mount as /docker-entrypoint-initdb.d/init.sql in Docker
-- ============================================================================
...
-- audit_logs indexes (most queried table — needs comprehensive indexing)
...
-- GIN index for full-text search on queries (optional, powerful)
-- CREATE INDEX IF NOT EXISTS idx_audit_logs_query_fts
--     ON audit_data.audit_logs USING GIN (to_tsvector('english', raw_query));
...
-- ============================================================================
-- PART 4: VIEWS (Convenient query interfaces)
-- ============================================================================

-- View: User activity summary (one row per user)
...
-- View: Hourly activity heatmap data (for dashboard heatmap)
...
-- View: Top queried tables (for dashboard bar chart)
...
-- View: Slow queries (queries above P95 threshold)
...
-- View: User operation mix (for stacked bar chart)
...
-- View: Anomaly timeline (for dashboard anomaly chart)
...
-- Function: Compute query hash (simplified — normalize literals)
...
BEGIN
    ...
    -- Application user: OLTP operations
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'app_user') THEN
        CREATE ROLE app_user WITH LOGIN PASSWORD 'app_password' NOSUPERUSER;
    END IF;

    -- Analyst role: read-only on audit data, read on working tables
    ...
    -- Night job role: batch processing
...
-- Working tables: public read/write (for load generator)
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO app_user;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO analyst;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO night_job;
GRANT ALL ON ALL TABLES IN SCHEMA public TO admin;

-- Grant sequence usage (for INSERT with SERIAL columns)
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO app_user;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO night_job;
GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO admin;

-- Audit data schema: analyst gets read-only
-- Note: In PostgreSQL, views are granted via ALL TABLES (no separate VIEWS grant)
GRANT SELECT ON ALL TABLES IN SCHEMA audit_data TO analyst;

-- App user can read/write audit logs (for direct loading)
...
-- Night job can read/write audit logs
